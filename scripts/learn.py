...
def generate_check_from_pattern(keyword_name, count, msg_sample):
    ...
    content_checks = {
        ...
        "faq": {
            ...
            "pattern": r"^faq:\s*$",
            "invert": True,
            "desc": "Article missing FAQ section in front matter",
        },
        "draft": {
            "target": "content",
            "pattern": r"^draft:\s*true\s*$",
            "desc": "Article in draft state",
        },
        "slug": {
            "target": "content",
            "pattern": r"slug:\s",
            "invert": True,
            "desc": "Article missing slug in front matter",
        },
        "seo": {
            "target": "content",
            "pattern": r"description:\s",
            "invert": True,
            "desc": "Article missing description in front matter",
        },
        "tag": {
            "target": "content",
            "pattern": r"^tags:",
            "invert": True,
            "desc": "Article missing tags in front matter",
        },
        "category": {
            "target": "content",
            "pattern": r"^categories:",
            "invert": True,
            "desc": "Article missing categories in front matter",
        },
        "image": {
            "target": "content",
            "pattern": r"cover:\s*\n\s+image:\s",
            "invert": True,
            "desc": "Article missing cover image in front matter",
        },
        "date": {
            "target": "templates",
    ...
    spec = content_checks.get(keyword_name)
    if not spec:
        return None
    ...
    desc = spec.get("desc", description)
    operator = "not found" if invert else "found"

    cond = "" if not invert else "not"

    if target == "content":
        code = textwrap.dedent(f'''\
        """Auto-generated by learn.py: {desc} (seen {count}x)"""
        import re
        from pathlib import Path
