...
| `mm` | `opencode` command | Workflow xuất bản news tiếng Hàn + TM + **toàn bộ ảnh từ bài gốc** | `.opencode/commands/mm.md` (fetch → TM → translate → **`fetch_cover.py --all`** → Hugo body gallery → glossary → QA → build). Bắt buộc cố gắng lấy **mọi** ảnh usable từ link gốc (`og:image` + body + CDN harvest) vào `static/images/YYYY/MM/` (`-cover`, `-01`, `-02`…), cover front matter + embed body | `implemented` | `.opencode/commands/mm.md`, `scripts/fetch_cover.py`, `scripts/glossary.py`, `data/glossary/`, `scientist.md` |
