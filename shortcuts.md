...
| `mm` | `opencode` command | Workflow xuất bản news tiếng Hàn + TM + **cover từ bài gốc** | `.opencode/commands/mm.md` (fetch → TM → translate → **`fetch_cover.py`** → Hugo → glossary → QA → build). Cover image: bắt buộc cố gắng lấy từ link gốc (`og:image` / body) vào `static/images/YYYY/MM/` | `implemented` | `.opencode/commands/mm.md`, `scripts/fetch_cover.py`, `scripts/glossary.py`, `data/glossary/`, `scientist.md` |
