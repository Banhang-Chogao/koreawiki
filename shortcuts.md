...
| `mm` | `opencode` command | Workflow xuất bản **news tiếng Hàn** + TM + **toàn bộ ảnh từ bài gốc** | `.opencode/commands/mm.md` (fetch → TM → KO→VI → **`fetch_cover.py --all`** → Hugo body gallery → glossary → QA → build). Nguồn: Dispatch, Hankyoreh, OhmyNews, … | `implemented` | `.opencode/commands/mm.md`, `scripts/fetch_cover.py`, `scripts/glossary.py`, `data/glossary/`, `scientist.md` |
| `nn` | `opencode` command | Workflow xuất bản **bài tiếng Anh** (báo thường **hoặc ArchDaily**) → VI + full gallery | `.opencode/commands/nn.md` — **cùng pipeline với `mm`** nhưng nguồn **English**; ArchDaily = feature kiến trúc + gallery. EN→VI, `fetch_cover.py --all`, body `![…](/images/…)` qua **render-image hook** (`baseURL` `/koreawiki/`), faq + footer, QA + smoke test HTML, push | `implemented` | `.opencode/commands/nn.md`, `themes/.../render-image.html`, `scripts/fetch_cover.py`, `scientist.md` |
